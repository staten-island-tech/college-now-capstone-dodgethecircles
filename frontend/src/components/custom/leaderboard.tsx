...
export default function Leaderboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const data = await fetch(`${process.env.BACKEND_URL}/leaderboard`).then(
          (res) => res.json()
        );
    ...
    useEffect(() => {
      async function fetchData() {
        try {
          const data = await fetch(`http://localhost:3000/leaderboard`).then(
            (res) => res.json()
          );
          setPlayers(await data);
        } catch (error) {
    ...
    return (
      <div className="bg-white rounded-lg p-3 h-80 w-64">
        ...
        <ScrollArea className="rounded-lg border h-5/6 w-full">
          {/* maybe have the number/crown for the players rank on top/above of the profile pic */}
          <div className="grid w-full grid-cols-3 content-baseline items-center text-center ">
            <div className="relative w-full">
              <div className="flex flex-col justify-center absolute z-10 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ">
                <CrownIcon color="gold" />
              </div>
              <Avatar className="opacity-80 mx-auto my-auto">
                <AvatarImage
                  src={sampleData.profilePicture || "@/public/default_pfp.svg"}
                />
                <AvatarFallback>CN</AvatarFallback>
              </Avatar>
            </div>
            <h4>{sampleData.username}</h4>
            <h4>{sampleData.highscore}</h4>
          </div>
          <div className="p-4">
            {/* Top 3 */}
            {players.length > 0 ? (
              ...
              >
                ...
                <Avatar>
                  <AvatarImage
                    src={
                      players[2].profilePicture || "@/public/default_pfp.svg"
                    }
                  />
                  <AvatarFallback>CN</AvatarFallback>
                </Avatar>
